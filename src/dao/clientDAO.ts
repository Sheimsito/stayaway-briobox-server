...
export const findAllClients = async (page: number = 1, limit: number = 10): Promise<{ clients: ClientRow[], total: number }> => {
  ...
  const { data, error, count } = await supabase
    .from('clients')
    .select('id, first_name, middle_name, paternal_last_name, maternal_last_name, age, email, phone, address', { count: 'exact' })
...
export const getAllFingerPrints = async (): Promise<{ fingerprints: Array<{ id: string; fingerprint_hash: string }> }> => {
  ...
  if (error) {
    ...
    throw new Error(`[clients] getAllFingerPrints: ${error.message}`);
  }
